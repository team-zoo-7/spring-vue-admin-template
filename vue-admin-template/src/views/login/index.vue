  ...
  <div class="login-container">
    ...
    >
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-tooltip content="[ 클릭 ] 갱신" placement="right" effect="light">
...
export default {
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (value.length < 5) {
        callback(new Error('사용자 이름은 5 자리 이상이어야합니다.'))
      } else {
        callback()
      }
    }
    const validatePass = (rule, value, callback) => {
      if (value.length < 5) {
        callback(new Error('암호는 5보다 작을 수 없습니다.'))
      } else {
        callback()
      }
    }
    const validateCodeText = (rule, value, callback) => {
      if (value.length !== 4) {
        callback(new Error('인증 코드는 4 자리 만 가능합니다.'))
      } else {
        callback()
      }
    }
